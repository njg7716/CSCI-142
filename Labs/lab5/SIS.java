    ...
    public void mainLoop(Scanner in, boolean stdin) {
        ...
        while (in.hasNext()) {
            ...
            if (fields[0].equals(HELP)) {
                ...
                System.out.println(backend.getCourse(id));
            } else if(fields[0].equals(ENROLL)){
                int id = Integer.parseInt(fields[2]);
                backend.enrollStudent(fields[1],id);
            } else if(fields[0].equals(PROFESSOR)){
                String username = fields[1];
                System.out.println(backend.getCoursesUser(username));
                ...
                String username = fields[1];
                int id = Integer.parseInt(fields[2]);
                backend.unenrollStudent(username,id);
            }else if(fields[0].equals(USERS)){
                System.out.println(backend.getAllUsers());
            }
            else {
