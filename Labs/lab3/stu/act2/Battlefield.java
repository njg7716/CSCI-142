...
public class Battlefield {
     ...
     */
    //Prints the number of hostages, soldiers, guerrillas and number of people rescued at that moment
    private void printStatistics() {
        System.out.println("Statistics: #"+enemyBase.getNumHostages()+" hostages remain, #"+bunker.getNumSoldiers()+
                " soldiers remain, #"+enemyBase.getNumGuerillas()+" guerillas remain, #"+chopper.getNumRescued()+" rescued");
    }
     ...
     */
    //The main part of the code that runs the simulation
    private void battle() {
        System.out.println("Get to the choppa!");
        while(enemyBase.getNumHostages()>0 && bunker.getNumSoldiers()>0) {

            printStatistics();
            Soldier s = bunker.deployNextSoldier();
            Hostage h = enemyBase.rescueHostage(s);
            while (h == null) {
                printStatistics();
                h = enemyBase.rescueHostage(bunker.deployNextSoldier());
            }
            System.out.println(h + " rescued from enemy base by soldier " + s);
            int roll = nextInt(1, 100);
            System.out.println(s + " encounters the predator who rolls a #" + roll);
            if (roll > predator.CHANCE_TO_BEAT_SOLDIER) {
                s.vicory(predator);
                predator.defeat(s);
                chopper.boardPassenger(h);
            ...
            roll = nextInt(1, 100);
            System.out.println(h + " encounters the predator who rolls a #" + roll);
